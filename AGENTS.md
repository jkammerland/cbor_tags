...
## Security Ownership And Finding Acceptance

Treat this section as a mandatory triage boundary, not general guidance. See
`SECURITY.md` for the corresponding public policy.

Application-supplied types, schemas, codecs, callbacks, allocators, and ranges
are trusted user code whose behavior defines the application protocol. A
statically typed recursive destination explicitly authorizes recursive decoding.
Resource exhaustion caused by input exercising that recursion belongs to the
application's admission and resource policy; it is not a library vulnerability
merely because input controls the recursion depth.

Do not report a library security finding when the reproducer requires user code
that directly introduces the reported behavior by:

- recursively calling the encoder or decoder;
- succeeding without consuming or producing the item required by its contract;
- allocating without an application-defined bound;
- throwing, terminating, blocking, or otherwise implementing the failure; or
- supplying dishonest range, iterator, allocator, extent, or lifetime semantics.

A contract-conforming customization may expose a library bug, but the finding
must identify behavior introduced by the library beyond the natural behavior of
the customization. Before accepting a security finding, answer all of these:

1. Which exact library-owned operation introduces the unsafe behavior?
2. Would the behavior remain without the user customization that appears in the
   reproducer?
...
5. Does the caller-owned framing, admission, or resource-limit contract already
   assign responsibility?

If questions 1 through 3 have no concrete answer, or question 4 or 5 assigns the
behavior to the application, exclude the finding from the library security
report. Input controlling execution depth does not make recursion library-owned
when the application's static schema introduced that recursion.
