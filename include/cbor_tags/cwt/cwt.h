...
struct cose_signature {
    ...
    template <typename Decoder> constexpr auto decode(Decoder &dec) {
        return dec(as_array{3}, protected_header, unprotected, signature);
    }
};
...
[[nodiscard]] inline byte_string copy_bytes(std::span<const std::byte> bytes) {
    return byte_string{bytes.begin(), bytes.end()};
}
...
[[nodiscard]] inline expected<byte_string, status_code> make_sign1_tbs(const cose_sign1 &message,
                                                                       std::span<const std::byte> external_aad = {},
                                                                       std::span<const std::byte> detached_payload = {}) {
...
[[nodiscard]] inline expected<void, status_code> validate_sign1_algorithm(const header_map &protected_header,
                                                                         const header_map &unprotected,
                                                                         algorithm expected) {
    if (unprotected.alg) {
        return unexpected<status_code>{status_code::error};
    }
