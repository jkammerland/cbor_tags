...
template <typename T> struct cbor_header_encoder {
    ...
    template <typename R> constexpr void encode(array_range<R> value) {
        auto &enc = detail::underlying<T>(this);
        if constexpr (std::ranges::sized_range<R>) {
            enc.encode_major_and_size(static_cast<std::uint64_t>(std::ranges::size(value.range_)),
                                      static_cast<typename T::byte_type>(0x80));
        } else {
            enc.appender_(enc.data_, static_cast<typename T::byte_type>(get_indefinite_start<as_indefinite_array>()));
        }

        for (auto &&item : value.range_) {
            enc.encode(item);
        }
    ...
    template <typename R> constexpr void encode(map_range<R> value) {
        auto &enc = detail::underlying<T>(this);
        if constexpr (std::ranges::sized_range<R>) {
            enc.encode_major_and_size(static_cast<std::uint64_t>(std::ranges::size(value.range_)),
                                      static_cast<typename T::byte_type>(0xA0));
        } else {
            enc.appender_(enc.data_, static_cast<typename T::byte_type>(get_indefinite_start<as_indefinite_map>()));
        }

        for (auto &&entry : value.range_) {
    ...
    template <typename R> constexpr void encode(bstr_range<R> value) {
        auto &enc = detail::underlying<T>(this);
        if constexpr (std::ranges::sized_range<R>) {
            enc.encode_major_and_size(static_cast<std::uint64_t>(std::ranges::size(value.range_)),
                                      static_cast<typename T::byte_type>(0x40));
            for (auto byte : value.range_) {
            ...
            chunk.reserve(value.chunk_size_);
            auto flush_chunk = [&] {
                if (chunk.empty()) {
                    return;
                }
            ...
            for (auto byte : value.range_) {
                ...
                if (chunk.size() == value.chunk_size_) {
                    flush_chunk();
                }
            }
            flush_chunk();
