  ...
  public:
    ...
    template <IsRangeOfCborValues T> constexpr status_code decode_definite_range(T &value, std::uint64_t length) {
        ...
        if constexpr (HasReserve<T>) {
            ...
            if constexpr (IsContiguous<InputBuffer> || std::ranges::sized_range<const InputBuffer>) {
                // A complete child takes at least one CBOR byte. This is an
                // allocation guard, not a structural validation: it only
                // bounds a header-directed reservation by known remaining
                // input. Maps need a key and a value for every entry.
                auto minimum_payload_size = length;
                if constexpr (IsMap<T>) {
                    if (length > std::numeric_limits<std::uint64_t>::max() / 2U) {
                        return status_code::incomplete;
                    }
                    minimum_payload_size = length * 2U;
                }

                if (minimum_payload_size != 0U) {
                    if constexpr (std::numeric_limits<size_type>::max() < std::numeric_limits<std::uint64_t>::max()) {
                        if (minimum_payload_size > static_cast<std::uint64_t>(std::numeric_limits<size_type>::max())) {
                            return status_code::incomplete;
                        }
                    }
                    if (reader_.empty(data_, static_cast<size_type>(minimum_payload_size - 1U))) {
                        return status_code::incomplete;
                    }
                }
