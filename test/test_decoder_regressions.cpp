...
TEST_CASE("decoder rejects fixed byte string targets that overwrite unread input") {
    SUBCASE("target overlaps the payload and the next item") {
        struct shifted_target_input {
            std::array<std::byte, 2> prefix;
            std::array<std::byte, 2> target;
        };
        static_assert(offsetof(shifted_target_input, target) == 2);
        static_assert(sizeof(shifted_target_input) == 4);

        shifted_target_input storage{
            .prefix = {std::byte{0x42}, std::byte{0x01}},
            .target = {std::byte{0x02}, std::byte{0x01}},
        };
        const auto original = storage;
        const auto input    = std::as_bytes(std::span{&storage, std::size_t{1}});
        auto       dec      = make_decoder(input);
        auto       result   = dec(storage.target);

        REQUIRE_FALSE(result);
        CHECK_EQ(result.error(), status_code::error);
        CHECK_EQ(storage.prefix, original.prefix);
        CHECK_EQ(storage.target, original.target);
    }

    SUBCASE("target overlaps only unread items") {
        struct unread_target_input {
            std::array<std::byte, 3> prefix;
            std::array<std::byte, 2> target;
        };
        static_assert(offsetof(unread_target_input, target) == 3);
        static_assert(sizeof(unread_target_input) == 5);

        unread_target_input storage{
            .prefix = {std::byte{0x42}, std::byte{0x01}, std::byte{0x02}},
            .target = {std::byte{0x01}, std::byte{0x02}},
        };
        const auto original = storage;
        const auto input    = std::as_bytes(std::span{&storage, std::size_t{1}});
        auto       dec      = make_decoder(input);
...
TEST_CASE("decoder permits fixed byte string overlap confined to consumed input") {
    struct consumed_target_input {
        std::array<std::byte, 2> target;
        std::array<std::byte, 2> suffix;
    };
    static_assert(offsetof(consumed_target_input, suffix) == 2);
    static_assert(sizeof(consumed_target_input) == 4);

    consumed_target_input storage{
        .target = {std::byte{0x42}, std::byte{0xAA}},
        .suffix = {std::byte{0xBB}, std::byte{0x01}},
    };
    const auto   input = std::as_bytes(std::span{&storage, std::size_t{1}});
    std::uint8_t trailing{};
    auto         dec = make_decoder(input);

    REQUIRE(dec(storage.target, trailing));
    CHECK_EQ(storage.target[0], std::byte{0xAA});
    CHECK_EQ(storage.target[1], std::byte{0xBB});
    CHECK_EQ(trailing, 1);
}
