...
| Type choices | RFC 8610/RFC 9682 `type = type1 *( "/" type1 )` | Yes | `CDDL groups choices in map keys and repeated item positions`; `CDDL emits native null-or-value shapes for unique pointers`; `CDDL emits shared pointer reference shapes directly` | `std::variant` renders as `A / B`; `std::optional<T>` renders as `T / null`. Choices are grouped when embedded in positions that require `type1`. CDDL can describe distinct shared-pointer choices, but runtime variants containing shared pointers require an application codec. |
...
| Shared pointers | CBOR tags 28 and 29 rendered with RFC 8610/RFC 9682 `#6.n(type)` | Yes | `CDDL emits shared pointer reference shapes directly`; `CDDL propagates shared pointer reference shapes through containers`; `cddl_shared_ptr_variant_tag_compile_fails`; `cddl_shared_ptr_variant_tag29_compile_fails`; `cddl_shared_ptr_variant_any_tag_compile_fails` | Compatible shared pointer types render directly as `null / #6.28(T) / #6.29(uint)`. CDDL describes shapes only; table indices, exact pointer types, and completion are runtime checks. |
