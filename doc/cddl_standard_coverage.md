...
| Type choices | RFC 8610/RFC 9682 `type = type1 *( "/" type1 )` | Yes | `CDDL groups choices in map keys and repeated item positions`; `CDDL emits native null-or-value shapes for unique pointers`; `CDDL emits shared pointer shapes through explicit roots` | `std::variant` renders as `A / B`; `std::optional<T>` renders as `T / null`. Choices are grouped when embedded in positions that require `type1`. Pointer-containing variants are accepted only when their CBOR wire shapes are distinct. |
...
| Shared pointers | CBOR tags 28, 29, and 296 rendered with RFC 8610/RFC 9682 `#6.n(type)` | Yes | `CDDL emits shared pointer shapes through explicit roots`; `CDDL distinguishes tagged and caller-table shared pointer roots`; `cddl_shared_ptr_nested_scope_compile_fails`; `cddl_shared_ptr_variant_tag_compile_fails`; `cddl_shared_ptr_variant_tag29_compile_fails`; `cddl_shared_ptr_variant_any_tag_compile_fails` | `shared_ptr_cddl<T>` renders a tag-296 root containing `null / #6.28(T) / #6.29(uint)`. `shared_ptr_unscoped_cddl<T>` omits tag 296 for caller-owned tables spanning calls. CDDL describes shapes only; table indices, types, and completion are runtime checks. Both wrappers are root-only. |
...
- `CDDL emits shared pointer shapes through explicit roots`
- `CDDL distinguishes tagged and caller-table shared pointer roots`
