...
| Type choices | RFC 8610/RFC 9682 `type = type1 *( "/" type1 )` | Yes | `CDDL groups choices in map keys and repeated item positions`; `CDDL emits shared graph pointer shapes through explicit smart pointer scope` | `std::variant` renders as `A / B`; `std::optional<T>` renders as `T / null`. Choices are grouped when embedded in positions that require `type1`. Default nullable smart pointer variants are rejected as a conservative generator limitation. `shared_graph_cddl<T>` allows one nullable smart pointer alternative when there is no array-shaped alternative and no tag 28/29 or catch-all tag collision. |
...
| Shared graph smart pointers | CBOR value-sharing tags 28/29 rendered with RFC 8610/RFC 9682 `#6.n(type)` | Yes | `CDDL emits shared graph pointer shapes through explicit smart pointer scope`; `cddl_shared_graph_variant_tag_compile_fails`; `cddl_shared_graph_variant_array_compile_fails` | `cbor::tags::ext::smart_ptr::shared_graph_cddl<T>` scopes schema generation so `std::shared_ptr<T>` renders as `[0] / #6.28(T) / #6.29(uint)`. CDDL describes the wire shape only; reference-table identity and reference existence remain runtime decoder-session checks. |
