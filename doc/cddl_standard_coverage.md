...
| CDDL identifiers | RFC 8610/RFC 9682 `typename = id`, `id` | Partial | `CDDL gives colliding C++ short names distinct rule names` | Generated names are sanitized and collision-safe. Requested single-field aliases reject collisions. Exact generated names are not API-stable. |
...
| Text and byte strings | RFC 8610 `tstr`, `bstr` | Yes | `CDDL no columns`; `CDDL emits typed containers and registers nested definitions once`; `CDDL emits bounded sizes for strings containers and range wrappers` | `std::string` maps to `tstr`; byte ranges map to `bstr`. `bounded_size<T, Min, Max>` renders string size controls such as `tstr .size (1..64)` or `bstr .size 4`. |
...
| Aggregate arrays | RFC 8610/RFC 9682 `[ group ]` | Yes | `CDDL emits RFC 8610 shapes for aggregate arrays and tag payloads`; `CDDL labels aggregate array fields when requested` | Multi-field aggregates render as fixed arrays. Single-field aggregates render as the single payload shape, matching encoder behavior. With named reflection, `CDDLOptions::label_array_fields` emits field labels and single-field aliases. |
...
| Sequence containers | RFC 8610/RFC 9682 occurrence `*` in array groups | Yes | `CDDL emits typed containers and registers nested definitions once`; `CDDL emits bounded sizes for strings containers and range wrappers` | Variable sequences render as `[* value]`; fixed-size `std::array`/static `std::span` render as `[N*N value]`. `bounded_size<T, Min, Max>` renders bounded occurrences such as `[1*3 value]`. |
| Maps | RFC 8610/RFC 9682 `{ group }`, `memberkey =>` | Yes | `CDDL emits typed containers and registers nested definitions once`; `CDDL groups choices in map keys and repeated item positions`; `CDDL emits bounded sizes for strings containers and range wrappers` | Map keys and values are typed. Choice keys are parenthesized so they fit `memberkey = type1 =>`. `bounded_size<T, Min, Max>` renders bounded map occurrences such as `{0*2 key => value}`. |
...
| Type choices | RFC 8610/RFC 9682 `type = type1 *( "/" type1 )` | Yes | `CDDL groups choices in map keys and repeated item positions`; `CDDL emits native null-or-value shapes for unique pointers`; `CDDL emits shared pointer reference shapes directly` | `std::variant` renders as `A / B`; `std::optional<T>` renders as `T / null`. Choices are grouped when embedded in positions that require `type1`. Distinct shared-pointer choices also decode directly; overlapping outer wire shapes are rejected at compile time. |
| Unique pointers | RFC 8610/RFC 9682 type choice | Yes | `CDDL emits native null-or-value shapes for unique pointers`; `smart_ptr_nested_null_compile_fails`; `cddl_optional_smart_ptr_compile_fails` | `std::unique_ptr<T>` renders as `T / null`, matching `unique_ptr_codec`. A pointee that also accepts CBOR `null` is rejected. An ordinary `std::optional` cannot directly contain a smart-pointer null state; named-map omission remains distinct and is supported. |
| Shared pointers | CBOR tags 28 and 29 rendered with RFC 8610/RFC 9682 `#6.n(type)` | Yes | `CDDL emits shared pointer reference shapes directly`; `CDDL propagates shared pointer reference shapes through containers`; `cddl_shared_ptr_variant_tag_compile_fails`; `cddl_shared_ptr_variant_tag29_compile_fails`; `cddl_shared_ptr_variant_any_tag_compile_fails` | Compatible shared pointer types render directly as `null / #6.28(T) / #6.29(uint)`. CDDL describes shapes only; table indices, exact pointer types, and completion are runtime checks. Registered polymorphic pointees require an application-provided schema. |
...
| RFC 8746 typed arrays | RFC 8746 scalar and structural tags rendered with RFC 8610/RFC 9682 `#6.n(type)` | Yes | `CDDL emits RFC 8746 typed-array extension shapes`; `rfc8746 bounded typed arrays enforce element counts` | Scalar typed arrays render as `#6.N(bstr)`. `bounded_size` on scalar typed arrays uses C++ element counts and renders byte-string `.size` controls; element alignment remains a runtime decode check. Structural `homogeneous_array` and `multi_dimensional_array` wrappers render their payload shapes; bounded structural wrappers are deferred. |
...
| CDDL control operators | RFC 8610/RFC 9682 `ctlop` | Partial | `CDDL emits bounded sizes for strings containers and range wrappers`; `CDDL emits RFC 8746 typed-array extension shapes` | `bounded_size` for text and byte strings emits `.size`; scalar typed arrays emit `.size` on their byte-string payload. Other controls such as `.bits`, `.regexp`, and `.cbor` are not generated. |
...
The current standard-sensitive regression tests live in `test/test_cddl.cpp`
and extension-specific files such as `test/test_cbor_typed_arrays.cpp`:
...
- `CDDL emits native null-or-value shapes for unique pointers`
- `CDDL emits shared pointer reference shapes directly`
- `CDDL propagates shared pointer reference shapes through containers`
- `cddl_unique_ptr_variant_overlap_compile_fails`
- `cddl_shared_ptr_variant_tag_compile_fails`
...
- `CDDL emits bounded sizes for strings containers and range wrappers`
- `CDDL emits RFC 8746 typed-array extension shapes`
- `rfc8746 bounded typed arrays enforce element counts`
