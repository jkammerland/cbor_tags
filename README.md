...
The generated C++20 fallback can reflect aggregates with mutable reference members when the aggregate is trivially copy-constructible. It rejects a shape that combines mutable references with non-trivially-copyable value members because C++20 cannot select lvalue and rvalue aggregate probes independently without instantiating potentially ill-formed copy constructors. Use native C++ reflection or a custom encoder/decoder for that shape.
